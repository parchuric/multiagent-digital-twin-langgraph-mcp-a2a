...
### Phase 3: Multi-Agent Communication Modes (Legacy vs MCP)

### Overview

This platform supports two agent-to-agent (A2A) communication modes:
...
**Note:** The MCP server and protocol are under development and not yet integrated into the platform. This phase documents the intended evolution of the communication architecture.
...
1. **Provision and Verify Core Azure Resources**
   ...
   Ensure the following Azure resources are provisioned and accessible:
...
## End-to-End Testing and Dashboard Visualization

### Overview
This release introduces a real-time, interactive dashboard for visualizing SCADA, PLC, and GPS events from all global manufacturing sites. The dashboard supports event type selection, map-based visualization, and detailed site/device status, enabling comprehensive monitoring and troubleshooting.

### What Was Tested
- **Global Facility Coverage**: All new facility locations (Ulsan, Pune, Hiroshima, Johannesburg, Dubai, Sao Paulo) were added to the platform and verified in the dashboard map.
- **Event Type Toggle**: The dashboard now features a dropdown to select SCADA, PLC, or GPS events, with dynamic map updates and color-coded status markers.
- **Data Flow Validation**: Simulators for SCADA, PLC, and GPS were updated to generate events for all `facility_ids`. The event stream processor ingests these events and archives them in Cosmos DB.
- **Map Rendering**: The dashboard uses Plotly to render a world map with facility/device markers, status colors, and hover details. Map size was optimized for UI clarity.
- **Site/Device Details**: Selecting an event type displays detailed status and event data for each site (SCADA/PLC) or device (GPS).

### How to Run the Complete Setup

#### Provision Azure Resources
Use Terraform scripts to deploy Event Hubs, Cosmos DB, and Key Vault.
...
* Copy `.env.sample` to `.env` and set required variables (Key Vault URI, Event Hub connection string, etc.).
* Ensure your Azure identity has data-plane permissions for all resources.
...
#### Start Simulators and Event Processor
...
Or run individual simulators and processors as needed:
...
Use the dropdown to select SCADA, PLC, or GPS events and view global site/device status.

### Troubleshooting

* If new facilities do not appear, ensure simulators are generating events for all `facility_ids` and Cosmos DB contains recent data.
* If the dashboard does not load, verify Python dependencies and environment variables.
* For Azure authentication issues, confirm you are logged in with `az login` and have correct RBAC permissions.

### Summary

This end-to-end test validates the full data pipeline:
**Simulators → Event Hubs → Event Processor → Cosmos DB → Dashboard**

The new dashboard capability provides real-time, global visibility into manufacturing operations, with flexible event type selection and detailed site/device insights.

> For further details, see the code comments in `dashboard.py` and the troubleshooting section above.
