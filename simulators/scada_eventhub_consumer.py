...
CONSUMER_GROUP = os.getenv('SCADA_EVENT_HUB_CONSUMER_GROUP', '$Default')

def on_event(partition_context, event):
    print(f"[SCADA] Partition: {partition_context.partition_id}")
...
with client:
    print(f"Listening for SCADA events on {EVENT_HUB_NAME}...")
    client.receive(
        on_event=on_event,
        starting_position="-1",
    )
