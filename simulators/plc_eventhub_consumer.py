...
CONSUMER_GROUP = os.getenv('PLC_EVENT_HUB_CONSUMER_GROUP', '$Default')

def on_event(partition_context, event):
    print(f"[PLC] Partition: {partition_context.partition_id}")
...
with client:
    print(f"Listening for PLC events on {EVENT_HUB_NAME}...")
    client.receive(
        on_event=on_event,
        starting_position="-1",
    )
